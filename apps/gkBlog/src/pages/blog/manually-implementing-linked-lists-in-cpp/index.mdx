...
而链表是非连续的内存空间，访问元素需要遍历链表（O(n)），但是在插入和删除元素时只需要修改指针，效率较高（O(1)）。
...
```cpp
template<typename T>
ListNode<T> *arrayToList(std::vector<T> arr) {
    if (arr.empty()) {
        return nullptr;
    }
...
```cpp
template<typename T>
DoublyListNode<T> *arrayToDoublyList(std::vector<T> arr) {
    if (arr.empty()) {
        return nullptr;
    }
...
// 先找到尾结点
for (DoublyListNode<int> *p = head; p->next_ != nullptr; p = p->next_) {
    tail = p->next_;
}
...
public:
    ...
    MyLinkedList() : size_(0) {
        head_ = new ListNode(T()); // 创建虚拟头结点
        tail_ = new ListNode(T()); // 创建虚拟尾结点
...
| `lst1.insert(lst1.begin(), 1)` | 在指定位置插入一个元素。                   | lst1 变为 `{1}`                           |
...
| `lst2.end()`                   | 返回指向列表末尾的迭代器。                 | 返回指向 10 的迭代器                      |
| `lst2.sort()`                  | 对列表中的元素进行排序。                   | lst2 变为 `{8, 9, 10}`                    |
| `lst2.reverse()`               | 反转列表中的元素顺序。                     | lst2 变为`{10, 9, 8}`                     |
| `lst1.merge(lst2)`             | 将另一个列表合并到当前列表中，并保持排序。 | lst1 变为` {1, 8, 9, 10}`，lst2 变为 `{}` |
